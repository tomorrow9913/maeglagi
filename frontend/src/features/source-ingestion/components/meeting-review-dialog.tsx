...
export function MeetingReviewDialog({ workspaceId, sourceId, onClose, onConfirmed, onRetried }: {
  ...
  return <Dialog open={Boolean(sourceId)} onOpenChange={(open) => { if (!open && !busy && (!dirty || window.confirm("저장하지 않은 변경을 버릴까요?"))) onClose(); }}><DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-3xl"><DialogHeader><DialogTitle>회의 대본 검토</DialogTitle><DialogDescription>{display?.readOnly ? display.waitingForAgent ? "확인된 대본입니다. 연결한 에이전트가 분석 결과를 저장할 때까지 기다립니다." : "확인된 대본과 프로젝트 정보는 유지됩니다." : reviewConfirmationCopy(review?.analysisMode).description}</DialogDescription></DialogHeader>
    {loading ? <p className="text-sm text-muted-foreground">대본을 불러오는 중…</p> : loadError ? <div role="alert" className="space-y-2 rounded-md border border-destructive p-3 text-sm"><p>{loadError}</p><Button size="sm" variant="outline" onClick={() => sourceId && void load(sourceId)}>다시 불러오기</Button></div> : !review ? <p className="text-sm text-muted-foreground">대본을 불러올 수 없습니다.<Button size="sm" variant="outline" onClick={() => sourceId && void load(sourceId)}>다시 시도</Button></p> : <div className="space-y-4">
      <p className="text-sm font-medium">{review.title} · {review.status === "awaiting_agent" ? "에이전트 작업 대기" : review.reviewState === "transcribing" ? review.status === "failed" ? "음성 인식 실패" : "서버 음성 인식 중" : isReadOnlyReview(review) ? "확인됨" : "검토 필요"}</p>
      {review.reviewState === "transcribing" ? review.status === "failed" ? <><div role="alert" className="space-y-2 rounded-md border border-destructive p-3 text-sm"><p>서버 음성 인식이 완료되지 않았습니다. 업로드한 녹음{review.utterances.length ? "과 실시간 대본 초안은" : "은"} 저장돼 있습니다.</p>{review.errorMessage && <p className="text-muted-foreground">{review.errorMessage}</p>}<div className="flex gap-2"><Button size="sm" disabled={busy || review.analysisMode === "agent"} onClick={() => void retryTranscription()}>{busy ? "다시 시도 중…" : "저장된 녹음으로 다시 시도"}</Button><Button size="sm" variant="outline" disabled={busy} onClick={() => sourceId && void load(sourceId)}>상태 새로고침</Button></div></div>{sourceId && review.analysisMode !== "agent" && <SavedRecordingRecovery key={sourceId} workspaceId={workspaceId} sourceId={sourceId} review={review} onSaved={(next) => { setReview(next); void load(sourceId); }} />}</> : <div className="space-y-2"><p className="text-sm text-muted-foreground">음성 인식이 끝나면 대본이 자동으로 갱신됩니다. 소스 목록에서도 다시 열 수 있습니다.</p><Button size="sm" variant="outline" onClick={() => sourceId && void load(sourceId, true)}>지금 새로고침</Button></div> : <>
        ...
        <fieldset className="space-y-1 text-sm"><legend className="font-medium">회의 프로젝트</legend><div className="flex flex-wrap gap-3">{projects.filter((item) => !item.archivedAt || projectIds.includes(item.id)).map((item) => <label key={item.id} className="flex items-center gap-1"><input type="checkbox" checked={projectIds.includes(item.id)} disabled={busy || isReadOnlyReview(review) || (Boolean(item.archivedAt) && !projectIds.includes(item.id))} onChange={(event) => { const next = event.target.checked ? [...projectIds, item.id] : projectIds.filter((id) => id !== item.id); setProjectIds(next); setProjectId(next[0] ?? ""); setDirty(true); }} />{item.name}{item.archivedAt ? " (보관됨)" : ""}</label>)}</div><p className="text-xs text-muted-foreground">프로젝트를 선택하지 않아도 대본은 확인할 수 있습니다.</p></fieldset>
        ...
        <div className="space-y-2"><p className="text-sm font-medium">새 발언 화자</p><div className="flex flex-wrap gap-1.5">{speakers.map((speaker) => <Button key={speaker.id} size="sm" variant="outline" disabled={isReadOnlyReview(review)} aria-pressed={activeSpeaker === speaker.id} className={activeSpeaker === speaker.id ? "ring-2 ring-current" : ""} onClick={() => setActiveSpeaker(speaker.id)}>{speaker.name}</Button>)}</div><Button size="sm" variant="ghost" disabled={isReadOnlyReview(review)} onClick={() => { const name = window.prompt("새 화자 이름"); if (name?.trim()) { const id = `local-${Date.now()}`; setSpeakers((values) => [...values, { id, name: name.trim() }]); setActiveSpeaker(id); } }}>미등록 화자 추가</Button></div>
        {speakers.filter((item) => item.id.startsWith("local-")).map((speaker) => <Input key={speaker.id} aria-label={`${speaker.name} 이름`} value={speaker.name} disabled={busy || isReadOnlyReview(review)} onChange={(event) => { setSpeakers((values) => values.map((item) => item.id === speaker.id ? { ...item, name: event.target.value } : item)); setDirty(true); }} />)}
        <TranscriptEditor rows={rows} speakers={speakers} activeSpeaker={activeSpeaker} disabled={busy || isReadOnlyReview(review)} onChange={(id, values) => { setRows((items) => items.map((item) => item.id === id ? { ...item, ...values } : item)); setDirty(true); }} onDelete={(id) => { setRows((items) => items.filter((item) => item.id !== id)); setDirty(true); }} onAdd={() => { const id = nextId.current--; setRows((items) => [...items, { id, speaker: activeSpeaker, text: "", isFinal: true, edited: true }]); setDirty(true); return id; }} />
        ...
        {review.reviewState === "awaiting_review" && <div className="flex flex-wrap gap-2"><Button variant="outline" disabled={busy || !dirty || conflict || invalidPersonRows.length > 0} onClick={() => void save()}>{busy ? "저장 중…" : "초안 저장"}</Button><Button disabled={busy || dirty || conflict || invalidPersonRows.length > 0 || !rows.some((row) => row.text.trim()) || projectIds.some((id) => Boolean(projects.find((item) => item.id === id)?.archivedAt))} onClick={() => void confirm()}>{reviewConfirmationCopy(review.analysisMode).button}</Button>{dirty && <span className="self-center text-xs text-muted-foreground">확인 전에 변경 내용을 저장해 주세요.</span>}</div>}
        {display?.showServerRetry && <div className="flex flex-wrap items-center gap-2 rounded-md border border-destructive p-3 text-sm"><span>대본은 확인됐지만 분석을 시작하지 못했습니다. 저장된 대본과 프로젝트 정보는 유지됩니다.</span><Button size="sm" disabled={busy} onClick={() => void confirm(true)}>분석 다시 시도</Button></div>}
