...
export function MeetingReviewDialog({ workspaceId, sourceId, onClose, onConfirmed }: {
  ...
  const load = useCallback(async (id: string, silent = false) => {
    ...
    try {
      ...
      const options: SpeakerOption[] = roster.map((item) => ({ id: item.id, name: `${item.name}${item.archivedAt ? " (보관됨)" : ""}` }));
      ...
      setProjectId(data.projectId || ""); setDirty(false); setConflict(false);
  ...
  const save = async () => {
    ...
    setBusy(true);
    try {
      const next = await api.saveMeetingReview(workspaceId, sourceId, { revision: review.revision, projectId: projectId || null, utterances: utterances() });
  ...
  const confirm = async (retry = false) => {
    if (!sourceId || !review || busy || (!retry && (dirty || !projectId || invalidPersonRows.length || !rows.some((row) => row.text.trim())))) return;
    setBusy(true);
