...
export function MeetingCapture({ workspaceId, onAudio, onTranscript, onBusyChange }: {
  ...
  return <div className="space-y-4">
    ...
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-2"><select aria-label="전체 작업공간에서 참여자 추가" defaultValue="" disabled={saving || directoryLoading || directoryLoadError} onChange={(event) => { const person = people.find((item) => item.id === event.target.value); if (person) addPerson(person); event.target.value = ""; }} className="rounded-md border bg-background px-2 py-1 text-sm"><option value="">전체 작업공간에서 참여자 추가</option>{people.filter((item) => !item.archivedAt && !speakers.some((speaker) => speaker.id === item.id)).map((item) => <option key={item.id} value={item.id}>{item.name}{item.role ? ` · ${item.role}` : ""}</option>)}</select><Input aria-label="이번 회의 임시 화자 이름" placeholder="이번 회의 임시 화자 이름" value={localName} maxLength={80} className="w-48" disabled={saving} onChange={(event) => setLocalName(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") addLocalSpeaker(); }} /><Button size="sm" variant="outline" disabled={saving} onClick={addLocalSpeaker}>임시 화자 추가</Button></div>
