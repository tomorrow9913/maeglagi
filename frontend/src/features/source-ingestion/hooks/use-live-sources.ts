...
export function useLiveSources(workspaceId: string) {
  const api = useApi();
  ...
  const settled = useRef<{ workspaceId: string; ids: Set<string> }>({ workspaceId, ids: new Set() });
  const invalidated = useRef<{ workspaceId: string; ids: Set<string> }>({ workspaceId, ids: new Set() });
  if (settled.current.workspaceId !== workspaceId) settled.current = { workspaceId, ids: new Set() };
  ...
  useEffect(() => {
    ...
    api.listSources(workspaceId, controller.signal).then((data) => {
      ...
      if (settled.current.workspaceId === workspaceId) for (const source of data) if (isPending(source.status)) settled.current.ids.delete(source.id);
  ...
  const onJob = useCallback((job: ProcessingJob) => {
    ...
    if (!isPending(job.status) && settled.current.workspaceId === workspaceId && !settled.current.ids.has(job.sourceId)) {
      settled.current.ids.add(job.sourceId);
      reload();
    }
  }, [reload, workspaceId]);

  useWorkspaceSourceEvents(workspaceId, sources?.filter((source) => isPending(source.status)).map((source) => source.id) ?? [], onJob, !isDemo, (sourceId) => {
