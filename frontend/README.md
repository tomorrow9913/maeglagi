...
`/login`의 로그인과 계정 만들기 화면에서 같은 카카오 OAuth 버튼을 사용합니다. 모든 계정에 이메일이 필요하므로 프론트는 Supabase `kakao` provider에 `account_email profile_nickname profile_image` 범위를 요청하고 기존 `/auth/callback`에서 세션을 교환합니다. `NEXT_PUBLIC_KAKAO_OAUTH_ENABLED`는 기본적으로 꺼져 있습니다. 카카오 로그인 버튼과 계정 연결 버튼은 이 값을 `true`로 빌드한 경우에만 표시됩니다.

대시보드 설정 시 Kakao Developers에서 앱을 비즈 앱으로 전환하고 `account_email`을 **필수 동의**로 설정합니다. 닉네임·프로필 이미지 동의, 로그인 기능, Supabase Auth Kakao provider의 REST API 키와 Client Secret도 설정합니다. Kakao Redirect URI에는 Supabase의 `https://<project-ref>.supabase.co/auth/v1/callback`을, Supabase Auth Redirect URLs에는 실제 프론트의 `https://<frontend-origin>/auth/callback`을 등록합니다(로컬 개발 주소도 사용할 경우 별도 등록). Supabase의 "Allow users without an email"은 끄고, 실제 계정으로 이메일이 반환되는 것을 검증한 뒤 Vercel Preview·Production의 `NEXT_PUBLIC_KAKAO_OAUTH_ENABLED=true`를 설정하여 재배포합니다. 비밀 키는 서버 측 대시보드에만 둡니다.
