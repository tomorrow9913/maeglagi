...
- When your agent creates a text source or uploads a file, the response includes `nextAction` with a tool name and source ID. The agent should continue that same turn: obtain the analysis context, extract grounded contexts, graph nodes and relations with its own model, and call `submit_analysis`. For media, it must first download and transcribe or parse the bytes; meetings still require explicit human transcript confirmation. Only `submit_analysis` returning `phase: done` means analysis is complete, and it returns a `completionMessage`. A registration response alone means the source is saved and waiting for the agent. MCP is request driven: the server cannot wake an idle client or run the client's model in the background. A client that uploads outside an active agent turn must resume the workflow itself.
