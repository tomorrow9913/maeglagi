...
Notion TSK-39의 완료 기준은 워크스페이스/BYOK, 문서·녹음 자동 분석, Timeline,
Graph, 근거 포함 Ask의 다섯 경로다. 아래 스모크는 이 중 API 경로를 점검하며
실제 녹음/STT와 브라우저 화면 검증을 대신하지 않는다.

TSK-49의 별도 조건인 “명령 한 번으로 데모 전체 상태를 1분 이내 복원”은
이 도구의 시드 재업로드로 충족되지 않는다. 재분석은 모델 응답 시간에 의존한다.
...
- 2026-09-19 Notion 상세 완료 기준을 직접 확인했다. Anthropic 점진적 스트리밍은
  PR #51에서 구현되었으며 중단/오류 처리를 포함한 자동 테스트가 있다.
- 분석기 네 시드 및 순차 Context Store 갱신 검증은 fixture provider 응답을 사용한다.
  실제 모델의 추출 정확도나 실서비스 저장소 연결을 검증한 것으로 해석하지 않는다.
- 남은 검증: 실제 환경의 다섯 완료 기준, 원본 녹음/STT, 그래프 육안 확인,
  스냅샷 기반 복원 시간 측정.
